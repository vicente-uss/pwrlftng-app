...
export function PwrlftngApp() {
  ...
  useEffect(() => { if (hydrated && activeSession) setScreen('active-session'); }, [hydrated, activeSession]);
  useEffect(() => {
    ...
    restoreSession().then(async session => {
      if (session) {
        ...
        if (active) setScreen(current => current === 'login' ? 'home' : current);
      }
    }).catch(() => undefined).finally(() => { if (active) setAuthChecked(true); });
    return () => { active = false; };
  }, [hydrated, initializeCloudSync, authChecked]);
  ...
  if (screen === 'login') return <LoginScreen onLogin={async () => { await initializeCloudSync(); setScreen('home'); }} onDemo={() => setScreen('home')} />;
