...
export type Screen = 'login' | 'account-type' | 'training' | 'create-routine' | 'edit-routine' | 'routine-detail' | 'active-session' | 'summary' | 'history' | 'session-detail' | 'profile' | 'exercise-library' | 'exercise-detail' | 'coach-athletes' | 'coach-athlete-detail' | 'coach-block-editor';
...
export type ActiveSet = { id: string; type: SetType; weight: string; reps: string; targetRepsMin: number; targetRepsMax: number; rpe: string; rir: string; completed: boolean; completedAt?: string; sourceRoutineSetId?: string; modificationType?: WorkoutModification };
